...
wine_dfnew=pd.read_csv('/Users/bonythomas/new.csv', usecols=['country','points','price','variety2','winery2'],encoding='latin-1')
wine_df=wine_dfnew
wine_df=wine_df.dropna(axis=0,how='any')

def clean_dataset(wine_train):
    assert isinstance(wine_train, pd.DataFrame), "df needs to be a pd.DataFrame"
    wine_train.dropna(inplace=True)
    indices_to_keep = ~wine_train.isin([np.nan, np.inf, -np.inf]).any(1)
    print(indices_to_keep)
    return wine_train[indices_to_keep].astype(np.int64)
...
country = LabelEncoder ().fit (np.unique (wine_df.country.values))
var_lab = LabelEncoder ().fit (np.unique (wine_df.variety2.values))
wnry_lab = LabelEncoder ().fit (np.unique (wine_df.winery2.values))
wine_train['country'] = country.transform (wine_train.country)
wine_train['var_enc'] = var_lab.transform (wine_train.variety2)
wine_train['wnry_enc'] = wnry_lab.transform (wine_train.winery2)
wine_train = wine_train.drop (['country', 'variety2', 'winery2'], axis=1)
target=['points']
# Prepare test data for classification tree
wine_test['country'] = country.transform (wine_test.country)
wine_test['var_enc'] = var_lab.transform (wine_test.variety2)
wine_test['wnry_enc'] = wnry_lab.transform (wine_test.winery2)
wine_test = wine_test.drop (['country', 'variety2', 'winery2'], axis=1)



clean_dataset(wine_train)
clean_dataset(wine_test)

# Train classification tree
x = wine_train.loc[:, ['price', 'country', 'var_enc', 'wnry_enc']]
y = wine_train['point_bins']
x_test = wine_test.loc[:, ['price', 'country', 'var_enc', 'wnry_enc']]
...
dtree = tree.DecisionTreeClassifier(criterion = "gini", splitter = 'random', max_leaf_nodes = 20, min_samples_leaf = 5, max_depth= 5)
dtree.fit(x, y)
prunetrain_error = y == dtree.predict (x)
dot_data = StringIO()
export_graphviz(dtree, out_file=dot_data,feature_names=class_names,
                filled=True, rounded=True,
                special_characters=True)
export_graphviz(dtree, out_file='wineDT1Pruned.dot')
graph = pydotplus.graph_from_dot_data(dot_data.getvalue())
graph.write_png("WineDTPruned.png")
print("Feature importance after pruning:")
print (pd.DataFrame ([dtree.feature_importances_], columns=x.columns.values))
Prunetest_error1 = wine_test['point_bins'] == dtree.predict(wine_test.loc[:, ['price','country', 'var_enc', 'wnry_enc']])
print ('@@@@@@@@@@@@@@@@@@@@@@@@@')
...
for j in [20,29,39]:
    clf_stump=DecisionTreeClassifier(criterion = "gini", splitter = 'random', max_leaf_nodes = j, min_samples_leaf = 5, max_depth= 5)
    for i in np.arange(1,max_n_ests):
        ...
        bag_tst_err=wine_test['point_bins']==baglfy.predict( \
            wine_test.loc[:,['price','country','var_enc','wnry_enc']])
...
clf_probs=clf1.predict_proba(wine_test.loc[:,['price','country','var_enc', \
    'wnry_enc']])
fpr1,tpr1,thr1=roc_curve(np.where(wine_test['point_bins']==0,1.,0.), \
    clf_probs[:,0])
clf_probs=dtree.predict_proba(wine_test.loc[:,['price','country','var_enc', \
    'wnry_enc']])
fpr2,tpr2,thr2=roc_curve(np.where(wine_test['point_bins']==0,1.,0.), \
    clf_probs[:,0])
# ROC curve for bagging ensemble using full classification trees
bag_probs=baglfy.predict_proba(wine_test.loc[:,['price','country', \
    'var_enc','wnry_enc']])
fpr3,tpr3,thr3=roc_curve(np.where(wine_test['point_bins']==0,1.,0.), \
    bag_probs[:,0])
# ROC curve for boosting ensemble using full classification trees
boost_probs=bstlfy.predict_proba(wine_test.loc[:,['price','country', \
    'var_enc','wnry_enc']])
fpr4,tpr4,thr4=roc_curve(np.where(wine_test['point_bins']==0,1.,0.), \
                         boost_probs[:,0])
...
print("The accuracy score of baseline CART is :",accuracy_score(wine_test['point_bins'], test_error1))
print("Accuracy of pruned CART:",accuracy_score(wine_test['point_bins'], Prunetest_error1))
print("Accuracy of bagged CART:",accuracy_score(wine_test['point_bins'], bag_tst_err))
print("Accuracy of boosted CART:",accuracy_score(wine_test['point_bins'], bst_tst_err))
